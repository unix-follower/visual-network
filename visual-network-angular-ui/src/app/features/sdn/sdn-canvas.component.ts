...
import { SdnCanvasRendererService } from "./sdn-canvas-renderer.service"
import { SdnTopology, SdnViewport } from "./sdn.models"

const DEFAULT_VIEWPORT: SdnViewport = {
  scale: 1,
  offsetX: 0,
  offsetY: 0,
}
...
export class SdnCanvasComponent implements AfterViewInit {
  readonly topology = input.required<SdnTopology>()
  ...
  readonly viewport = signal<SdnViewport>(DEFAULT_VIEWPORT)
  readonly canvasAriaLabel = computed(() => {
    const viewport = this.viewport()
    return `${this.topology().name} SDN topology canvas. Zoom ${Math.round(viewport.scale * 100)} percent.`
  })

  @ViewChild("canvas", { static: true })
  private readonly canvasRef?: ElementRef<HTMLCanvasElement>

  private readonly renderer = inject(SdnCanvasRendererService)
