...
import { SanCanvasRendererService } from "./san-canvas-renderer.service"
import { SanTopology, SanViewport } from "./san.models"

const DEFAULT_VIEWPORT: SanViewport = {
  scale: 1,
  offsetX: 0,
  offsetY: 0,
}
...
export class SanCanvasComponent implements AfterViewInit {
  readonly topology = input.required<SanTopology>()
  ...
  readonly viewport = signal<SanViewport>(DEFAULT_VIEWPORT)
  readonly canvasAriaLabel = computed(() => {
    const viewport = this.viewport()
    return `${this.topology().name} SAN topology canvas. Zoom ${Math.round(viewport.scale * 100)} percent.`
  })

  @ViewChild("canvas", { static: true })
  private readonly canvasRef?: ElementRef<HTMLCanvasElement>

  private readonly renderer = inject(SanCanvasRendererService)
