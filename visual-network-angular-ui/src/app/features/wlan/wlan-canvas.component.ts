...
import { WlanCanvasRendererService } from "./wlan-canvas-renderer.service"
import { WlanTopology, WlanViewport } from "./wlan.models"

const DEFAULT_VIEWPORT: WlanViewport = {
  scale: 1,
  offsetX: 0,
  offsetY: 0,
}
...
export class WlanCanvasComponent implements AfterViewInit {
  readonly topology = input.required<WlanTopology>()
  readonly selectedDeviceId = input<string | null>(null)
  readonly highlightedAccessPointId = input<string | null>(null)
  readonly activePathConnectionIds = input<string[]>([])
  readonly deviceSelected = output<string | null>()
  readonly viewport = signal<WlanViewport>(DEFAULT_VIEWPORT)
  readonly canvasAriaLabel = computed(() => {
    const viewport = this.viewport()
    return `${this.topology().name} wireless topology canvas. Zoom ${Math.round(viewport.scale * 100)} percent.`
  })

  @ViewChild("canvas", { static: true })
  private readonly canvasRef?: ElementRef<HTMLCanvasElement>

  private readonly renderer = inject(WlanCanvasRendererService)
  ...
  constructor() {
    effect(() => {
      ...
      const highlightedAccessPointId = this.highlightedAccessPointId()
