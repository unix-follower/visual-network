...
import { WanCanvasRendererService } from "./wan-canvas-renderer.service"
import { WanTopology, WanViewport } from "./wan.models"

const DEFAULT_VIEWPORT: WanViewport = {
  scale: 1,
  offsetX: 0,
  offsetY: 0,
}
    ...
    <div class="canvas-shell">
      <div class="toolbar" aria-label="Canvas controls">
        ...
        <p class="toolbar-copy">
          Use drag to pan, wheel to zoom, arrows to change site, Shift+arrows to move view.
        </p>
      </div>
...
export class WanCanvasComponent implements AfterViewInit {
  readonly topology = input.required<WanTopology>()
  ...
  readonly viewport = signal<WanViewport>(DEFAULT_VIEWPORT)
  readonly canvasAriaLabel = computed(() => {
    const viewport = this.viewport()
    return `${this.topology().name} WAN topology canvas. Zoom ${Math.round(viewport.scale * 100)} percent.`
  })

  @ViewChild("canvas", { static: true })
  private readonly canvasRef?: ElementRef<HTMLCanvasElement>

  private readonly renderer = inject(WanCanvasRendererService)
