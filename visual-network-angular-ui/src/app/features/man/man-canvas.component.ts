...
import { ManCanvasRendererService } from "./man-canvas-renderer.service"
import { ManTopology, ManViewport } from "./man.models"

const DEFAULT_VIEWPORT: ManViewport = {
  scale: 1,
  offsetX: 0,
  offsetY: 0,
}
...
export class ManCanvasComponent implements AfterViewInit {
  readonly topology = input.required<ManTopology>()
  ...
  readonly viewport = signal<ManViewport>(DEFAULT_VIEWPORT)
  readonly canvasAriaLabel = computed(() => {
    const viewport = this.viewport()
    return `${this.topology().name} MAN topology canvas. Zoom ${Math.round(viewport.scale * 100)} percent.`
  })

  @ViewChild("canvas", { static: true })
  private readonly canvasRef?: ElementRef<HTMLCanvasElement>

  private readonly renderer = inject(ManCanvasRendererService)
