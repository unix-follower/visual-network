...
import { ManConnection, ManDevice, ManTopology, ManViewport } from "./man.models"

@Injectable({ providedIn: "root" })
export class ManCanvasRendererService extends TopologyCanvasRendererService<
  ManTopology,
  ManDevice,
  ManConnection,
  ManViewport
> {
  ...
  private drawConnection(
    ...
    if (connection.kind === "metro-fiber") {
  ...
  private deviceAbbreviation(kind: ManDevice["kind"]): string {
    switch (kind) {
      case "metro-core":
        return "MC"
  ...
  private deviceFill(kind: ManDevice["kind"]): string {
    switch (kind) {
      case "metro-core":
        return "#4fd1c5"
      case "building-distribution":
  ...
  private statusFill(status: ManDevice["status"]): string {
    switch (status) {
      case "online":
        return "#68d391"
      case "degraded":
  ...
  private connectionStroke(connection: ManConnection): string {
    let alpha = "0.45"
