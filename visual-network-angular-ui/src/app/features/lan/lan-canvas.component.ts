...
import { LanCanvasRendererService } from "./lan-canvas-renderer.service"
import { LanTopology, LanViewport } from "./lan.models"

const DEFAULT_VIEWPORT: LanViewport = {
  scale: 1,
  offsetX: 0,
  offsetY: 0,
}
...
export class LanCanvasComponent implements AfterViewInit {
  readonly topology = input.required<LanTopology>()
  readonly selectedDeviceId = input<string | null>(null)
  readonly deviceSelected = output<string | null>()
  readonly viewport = signal<LanViewport>(DEFAULT_VIEWPORT)
  ...
  private readonly renderer = inject(LanCanvasRendererService)
