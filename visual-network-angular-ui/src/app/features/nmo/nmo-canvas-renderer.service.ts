...
import { NmoConnection, NmoDevice, NmoTopology, NmoViewport } from "./nmo.models"

@Injectable({ providedIn: "root" })
export class NmoCanvasRendererService extends TopologyCanvasRendererService<
  NmoTopology,
  NmoDevice,
  NmoConnection,
  NmoViewport
> {
  ...
  private drawConnection(
    ...
    if (connection.kind === "primary-link") {
      context.setLineDash([])
    } else if (connection.kind === "backup-link") {
      context.setLineDash([10, 6])
    } else if (connection.kind === "control-link") {
    ...
    context.fillText(
      `${connection.remediationAction} · ${connection.health} · ${connection.latencyMs} ms`,
      middleX,
      middleY - 10,
    )
    context.restore()
  ...
  private deviceAbbreviation(kind: NmoDevice["kind"]): string {
    switch (kind) {
      case "managed-node":
        return "ND"
      case "automation-controller":
        return "CT"
      case "health-monitor":
        return "HM"
      case "backup-gateway":
  ...
  private deviceFill(kind: NmoDevice["kind"]): string {
    switch (kind) {
      case "managed-node":
        return "#93c5fd"
      case "automation-controller":
  ...
  private statusFill(status: NmoDevice["status"]): string {
    switch (status) {
      case "online":
        return "#68d391"
      case "degraded":
  ...
  private connectionStroke(connection: NmoConnection): string {
    const alpha =
