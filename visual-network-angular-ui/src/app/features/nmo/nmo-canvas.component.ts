...
import { NmoCanvasRendererService } from "./nmo-canvas-renderer.service"
import { NmoTopology, NmoViewport } from "./nmo.models"

const DEFAULT_VIEWPORT: NmoViewport = {
  scale: 1,
  offsetX: 0,
  offsetY: 0,
}
...
export class NmoCanvasComponent implements AfterViewInit {
  readonly topology = input.required<NmoTopology>()
  ...
  readonly viewport = signal<NmoViewport>(DEFAULT_VIEWPORT)
  readonly canvasAriaLabel = computed(() => {
    const viewport = this.viewport()
    return `${this.topology().name} automation topology canvas. Zoom ${Math.round(viewport.scale * 100)} percent.`
  })

  @ViewChild("canvas", { static: true })
  private readonly canvasRef?: ElementRef<HTMLCanvasElement>

  private readonly renderer = inject(NmoCanvasRendererService)
