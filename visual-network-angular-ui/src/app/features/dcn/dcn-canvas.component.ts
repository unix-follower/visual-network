...
import { DcnCanvasRendererService } from "./dcn-canvas-renderer.service"
import { DcnTopology, DcnViewport } from "./dcn.models"

const DEFAULT_VIEWPORT: DcnViewport = {
  scale: 1,
  offsetX: 0,
  offsetY: 0,
}
...
export class DcnCanvasComponent implements AfterViewInit {
  readonly topology = input.required<DcnTopology>()
  ...
  readonly viewport = signal<DcnViewport>(DEFAULT_VIEWPORT)
  readonly canvasAriaLabel = computed(() => {
    const viewport = this.viewport()
    return `${this.topology().name} DCN topology canvas. Zoom ${Math.round(viewport.scale * 100)} percent.`
  })

  @ViewChild("canvas", { static: true })
  private readonly canvasRef?: ElementRef<HTMLCanvasElement>

  private readonly renderer = inject(DcnCanvasRendererService)
