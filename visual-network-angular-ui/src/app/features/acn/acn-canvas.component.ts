...
import { AcnCanvasRendererService } from "./acn-canvas-renderer.service"
import { AcnTopology, AcnViewport } from "./acn.models"

const DEFAULT_VIEWPORT: AcnViewport = {
  scale: 1,
  offsetX: 0,
  offsetY: 0,
}
...
export class AcnCanvasComponent implements AfterViewInit {
  readonly topology = input.required<AcnTopology>()
  ...
  readonly viewport = signal<AcnViewport>(DEFAULT_VIEWPORT)
  readonly canvasAriaLabel = computed(() => {
    const viewport = this.viewport()
    return `${this.topology().name} security topology canvas. Zoom ${Math.round(viewport.scale * 100)} percent.`
  })

  @ViewChild("canvas", { static: true })
  private readonly canvasRef?: ElementRef<HTMLCanvasElement>

  private readonly renderer = inject(AcnCanvasRendererService)
