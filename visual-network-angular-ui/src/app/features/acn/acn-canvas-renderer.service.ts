...
import { AcnConnection, AcnDevice, AcnTopology, AcnViewport } from "./acn.models"

@Injectable({ providedIn: "root" })
export class AcnCanvasRendererService extends TopologyCanvasRendererService<
  AcnTopology,
  AcnDevice,
  AcnConnection,
  AcnViewport
> {
  ...
  private drawConnection(
    ...
    if (connection.kind === "allowed-link") {
      context.setLineDash([])
    } else if (connection.kind === "inspection-link") {
    ...
    context.fillText(
      `${connection.ruleId} · ${connection.action} · ${connection.latencyMs} ms`,
      middleX,
      middleY - 10,
    )
    context.restore()
  ...
  private deviceAbbreviation(kind: AcnDevice["kind"]): string {
    switch (kind) {
      case "source-host":
        return "SR"
      case "access-firewall":
        return "FW"
      case "service-host":
        return "SV"
      case "management-station":
        return "MG"
      default:
        return "ND"
    }
  ...
  private deviceFill(kind: AcnDevice["kind"]): string {
    switch (kind) {
      case "source-host":
        return "#93c5fd"
      case "access-firewall":
  ...
  private statusFill(status: AcnDevice["status"]): string {
    switch (status) {
      case "online":
        return "#68d391"
      case "degraded":
