...
import { CenCanvasRendererService } from "./cen-canvas-renderer.service"
import { CenTopology, CenViewport } from "./cen.models"

const DEFAULT_VIEWPORT: CenViewport = {
  scale: 1,
  offsetX: 0,
  offsetY: 0,
}
...
export class CenCanvasComponent implements AfterViewInit {
  readonly topology = input.required<CenTopology>()
  ...
  readonly viewport = signal<CenViewport>(DEFAULT_VIEWPORT)
  readonly canvasAriaLabel = computed(() => {
    const viewport = this.viewport()
    return `${this.topology().name} cloud and edge topology canvas. Zoom ${Math.round(viewport.scale * 100)} percent.`
  })

  @ViewChild("canvas", { static: true })
  private readonly canvasRef?: ElementRef<HTMLCanvasElement>

  private readonly renderer = inject(CenCanvasRendererService)
